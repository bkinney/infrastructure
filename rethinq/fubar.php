...
if($_POST['members']){//members have been selected
 ...
 $cheaters = array();
	if(count($cheaters)){
		...
		if(mysqli_query($link,$query,$link)){
...
function initQuiz($overwrite,$display){
	...
	/*				if(!$answer['html']){
			...
			echo '<p align="right">';
			if(!$_SESSION['cant_go_back'] && $qnum > 1)echo $qnum . '<button onclick="nextQuestion(-1)" class="udats-score" >Previous</button>';
			
			if($current_question_num < count($quiz)-1)	echo '<button onclick="nextQuestion(1)" class="udats-score" >Next</button></p>';
			
			if(!$one_at || $current_question_num == count($quiz)-1){
				echo '<button onclick="submitScore()" class="udats-score" >Submit Score</button></p>';
			}
		}
	}
}
...
<div id="questions" class="assessing" <?php echo $questionstyle ?> >
