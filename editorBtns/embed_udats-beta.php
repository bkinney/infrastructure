...
/*putting the contents of the mini-editor into the proper widget environment*/	
	...
	switch(type){
		case "accordion":
			...
			.append('<div ><p style="display:none">---- COMMENT begin ' + tabTitle + ' content. Do not delete. Users will not see comments ---------</p><p>'+ tabTitle + ' content goes here.</p><p style="display:none">---- COMMENT end ' + tabTitle + ' content. Do not delete. Users will not see comments ---------</p></div>');
   			 });
		...
		case "addtabs":
			...
			$("#tabsui div.udats-tabs").append('<div id="tabs-'+tabCounter+'"><p style="display:none">---- COMMENT begin ' + tabTitle + ' content. Do not delete. Users will not see comments ---------</p><p>' + tabTitle + ' content goes here.</p><p style="display:none">---- COMMENT end ' + tabTitle + ' content. Do not delete.---------</p></div>');
   			 });
			var tabscomment ='<!--<li><a href="#extra-1">Spare Tab 1></a></li><li><a href="#extra-1">Spare Tab 2></a></li> Uncomment these  to add new tabs after import is complete. You must do the same below the closing ul tag to reveal the associated div--></ul><!-- <div id="extra-1"> Content for Spare Tab 1 tab goes here</div><div id="extra-2"> Content for Spare Tab 2 tab goes here</div>-->';
			var thtml=$("#tabsui").html().replace("</ul>",tabscomment);
		...
		if($("#editor p:first a").length == 0){
			...
			$("#editor a").attr("title",settings).addClass("udats-dialog");
		...
		case "udats_mm":
			...
			//var id = $("#editor a").attr("href").substr(1);
			var settings = "modal:false|position:" + $("#dialog_mm_options").val();
			$("#editor a").attr("title",settings).addClass("udats-dialog");
			//$("#editor div").attr("id","udats_mm");
			url += encodeURI($("#editor").html());
			$("#url").val(url);
		break;
		default:
...
<div class="udats_mm instructions">
<p>This widget will create module-specific menus based on links you add from the Canvas Module links menu. The created menus should open automatically, but can be closed by students. To re-open the menu, students can use the link provided.</p><p>Currently you can only display one module menu per Canvas Page using this tool.</p>
  <ol><li>[optional] Replace the words "Show Module Menu" with the text you want users to click on to launch the overlay.</li>
    <li>By default, your menu will open at the top left corner of the browser window, but can be dragged to any other location by the user.. To switch the default position, use the white select menu in the editor toolbar. The menu's position will be fixed, meaning that it will remain onscreen even when the page is scrolled. </li>
  <li>Click the done 'Done' button located beneath the editor.</li>
  <li>Using the Canvas editor, delete the default text, and add Module links using Modules pane of the Insert Content panel (right side of your page editing screen).</li>
</ol>
</div>
...
<div class="glossary menu addtabs dialog_ext dialog_id accordion udats_mm" >
    ...
    <option value="left bottom">bottom left</option>
      ...
      <select id="dialog_mm_options" class="udats_mm" >
   <option value="left top">top left</option>
    <option value="right top">top right</option>
    ...
    <input type="submit" onClick="wrapAs()" value="test" style="float:right">
    </div><!-- toolset  -->
...
<div class="udats-tabs">
   <ul >
  <li>Tab 1</li>
  <!--<li><a href="#extra-1">Spare Tab 1></a></li>
  <li><a href="#extra-1">Spare Tab 2></a></li> 
  Uncomment these  to add new tabs after import is complete. You must do the same below the closing ul tag to reveal the associated div-->
</ul>
<!-- <div id="extra-1"> Content for Spare Tab 1 tab goes here</div>
<div id="extra-2"> Content for Spare Tab 2 tab goes here</div>
-->
</div><!-- end addtabs -->
...
<div id="udats_mm" class="udats_mm">

<p><a href="#udats_mm" target="udats-dialog">Show Module Menu</a></p>
<div id="udats_mm">Module Menu (after returning to Canvas, replace this text with a link to the module menu you require)</div>
</div>
</div>
...
<ol>
<li>Always list the correct answer first. Responses will be randomized each time a student takes the self-test.
<ol>
<li>Answer-specific feedback goes here. This is optional.</li>
</ol>
</li>
<li>This is an incorrect response
<ol>
<li>Feedback for this response</li>
</ol>
</li>
<li>Another incorrect answer
<ol>
<li>Feedback</li>
</ol>
</li>
