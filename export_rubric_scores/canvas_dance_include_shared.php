...
$context = new BLTI($secret, true, false);//try redirect
	...
	$shared = array_key_exists('custom_shared',$context->info);
		if($temptoken && !$_SESSION['token']){//go get a token
	...
	//die(" trying to score a temp token");
		...
		}else if($_SESSION['token']){//use whatever I've already got from previous trips
	...
	if(!$valid){
	if(!$link) include "/home/bkinney/includes/lti_db.php";
		if($shared ){
		...
		//echo $query;
		$result = mysql_query($query);//overwrite result
		...
		if(mysql_num_rows($result)==1){
			...
			$tokenstatus = "no matching rows " .$result . " " . mysql_num_rows($result);
		}
	}
